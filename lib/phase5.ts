...
export interface EventOpsProfile {
  advancedEventOps: Record<string, string[]>;
}
...
export async function getEventOpsProfile(): Promise<EventOpsProfile> {
  if (!auth?.currentUser || !db) {
    return { advancedEventOps: {} };
  }

  const snapshot = await getDoc(doc(db, "eventOpsProfiles", auth.currentUser.uid));
  const data = snapshot.exists() ? (snapshot.data() as Record<string, unknown>) : {};
  const advancedEventOps = (data.advancedEventOps as Record<string, unknown> | undefined) ?? {};

  return {
    advancedEventOps: Object.fromEntries(
      Object.entries(advancedEventOps).map(([key, value]) => [
        key,
...
export async function saveEventOpsProfile(input: EventOpsProfile) {
  const user = assertSignedIn();
  await setDoc(
    doc(db!, "eventOpsProfiles", user.uid),
    {
      ownerId: user.uid,
      advancedEventOps: Object.fromEntries(
        Object.entries(input.advancedEventOps).map(([key, value]) => [
          key,
