...
export async function getOpenBrandCampaigns() {
  ...
  const snapshot = await getDocs(
    query(collection(db, "brandCampaigns"), where("active", "==", true), orderBy("createdAt", "desc"), limit(30))
  );
...
export async function getOwnedBrandCampaigns() {
  ...
  const snapshot = await getDocs(
    query(collection(db!, "brandCampaigns"), where("ownerId", "==", auth!.currentUser!.uid), orderBy("createdAt", "desc"), limit(30))
  );
...
export async function getOwnedCampaignApplications() {
  ...
  const snapshot = await getDocs(
    query(collection(db!, "campaignApplications"), where("ownerId", "==", auth!.currentUser!.uid), orderBy("createdAt", "desc"), limit(50))
  );
...
export async function getCurrentUserDeliverables() {
  ...
  return snapshot.docs.map((docSnapshot: { id: string; data: () => Record<string, unknown> }) => {
    ...
    return {
      ...
      title: String(data.title ?? ""),
      status:
        data.status === "submitted" || data.status === "approved" ? data.status : "todo",
...
export async function getCurrentAmbassadorStats(): Promise<AmbassadorStats> {
  ...
  const invites = snapshot.docs.map((docSnapshot: { data: () => Record<string, unknown> }) => docSnapshot.data());
  const topEntry = invites
    .map((entry: Record<string, unknown>) => ({
      code: String(entry.code ?? ""),
      referralCount: Number(entry.referralCount ?? entry.uses ?? 0),
    }))
    .sort((left: { code: string; referralCount: number }, right: { code: string; referralCount: number }) => right.referralCount - left.referralCount)[0];

  return {
    invitesCreated: invites.length,
    referralCount: invites.reduce((sum: number, entry: Record<string, unknown>) => sum + Number(entry.referralCount ?? entry.uses ?? 0), 0),
...
export async function getCurrentPayoutSnapshot(): Promise<PayoutSnapshot> {
  ...
  const [invoices, bookings, tips, ambassador] = await Promise.all([
  ...
  const paidInvoices = invoices.filter((invoice: InvoiceRecord) => invoice.status === "paid").length;
  const bookingRevenue = bookings.docs.filter((docSnapshot: { data: () => Record<string, unknown> }) => {
    const data = docSnapshot.data();
    return data.status === "accepted" || data.status === "completed";
  }).length * 25;
  const tipRevenue = tips.docs.length * 10;

  return {
    estimatedPayout: paidInvoices * 50 + bookingRevenue + tipRevenue,
    paidInvoices,
    bookingRevenue,
    tipRevenue,
