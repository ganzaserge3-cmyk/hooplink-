...
export interface AdminWorkspaceProfile {
  advancedAdminOps: Record<string, string[]>;
}
...
export async function getAdminWorkspaceProfile(): Promise<AdminWorkspaceProfile> {
  if (!auth?.currentUser || !db) {
    return { advancedAdminOps: {} };
  }

  const snapshot = await getDoc(doc(db, "adminWorkspaceProfiles", auth.currentUser.uid));
  const data = snapshot.exists() ? (snapshot.data() as Record<string, unknown>) : {};
  const advancedAdminOps = (data.advancedAdminOps as Record<string, unknown> | undefined) ?? {};

  return {
    advancedAdminOps: Object.fromEntries(
      Object.entries(advancedAdminOps).map(([key, value]) => [
        key,
...
export async function saveAdminWorkspaceProfile(input: AdminWorkspaceProfile) {
  if (!auth?.currentUser || !db) {
    throw new Error("You must be signed in.");
  }

  await setDoc(
    doc(db, "adminWorkspaceProfiles", auth.currentUser.uid),
    {
      ownerId: auth.currentUser.uid,
      advancedAdminOps: Object.fromEntries(
        Object.entries(input.advancedAdminOps).map(([key, value]) => [
          key,
  ...
  await writeAuditLog({
    ...
    targetType: "admin_workspace",
    targetId: auth.currentUser.uid,
    summary: "Updated advanced admin workspace controls.",
  });
}
