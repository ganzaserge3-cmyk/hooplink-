...
export interface FanOpsProfile {
  advancedFanMeta: Record<string, string[]>;
}
...
export async function getFanOpsProfile(): Promise<FanOpsProfile> {
  if (!auth?.currentUser || !db) {
    return { advancedFanMeta: {} };
  }

  const snapshot = await getDoc(doc(db, "fanOpsProfiles", auth.currentUser.uid));
  const data = snapshot.exists() ? (snapshot.data() as Record<string, unknown>) : {};
  const advancedFanMeta = (data.advancedFanMeta as Record<string, unknown> | undefined) ?? {};

  return {
    advancedFanMeta: Object.fromEntries(
      Object.entries(advancedFanMeta).map(([key, value]) => [
        key,
...
export async function saveFanOpsProfile(input: FanOpsProfile) {
  const user = requireUser();
  await setDoc(
    doc(db!, "fanOpsProfiles", user.uid),
    {
      ownerId: user.uid,
      advancedFanMeta: Object.fromEntries(
        Object.entries(input.advancedFanMeta).map(([key, value]) => [
          key,
