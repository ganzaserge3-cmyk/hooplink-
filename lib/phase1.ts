...
export interface BrandMarketingProfile {
  advancedBrandOps: Record<string, string[]>;
}
...
export async function getBrandMarketingProfile(): Promise<BrandMarketingProfile> {
  if (!auth?.currentUser || !db) {
    return { advancedBrandOps: {} };
  }

  const snapshot = await getDoc(doc(db, "brandMarketingProfiles", auth.currentUser.uid));
  const data = snapshot.exists() ? (snapshot.data() as Record<string, unknown>) : {};
  const advancedBrandOps = (data.advancedBrandOps as Record<string, unknown> | undefined) ?? {};

  return {
    advancedBrandOps: Object.fromEntries(
      Object.entries(advancedBrandOps).map(([key, value]) => [
        key,
...
export async function saveBrandMarketingProfile(input: BrandMarketingProfile) {
  const user = requireUser();
  await setDoc(
    doc(db!, "brandMarketingProfiles", user.uid),
    {
      ownerId: user.uid,
      advancedBrandOps: Object.fromEntries(
        Object.entries(input.advancedBrandOps).map(([key, value]) => [
          key,
