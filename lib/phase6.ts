...
export interface GrowthProfile {
  ...
  advancedGrowthOps: Record<string, string[]>;
}
...
export async function getCurrentGrowthProfile() {
  ...
  return {
    ...
    advancedGrowthOps: Object.fromEntries(
      Object.entries((growth.advancedGrowthOps as Record<string, unknown> | undefined) ?? {}).map(([key, value]) => [
        key,
  ...
  await setDoc(
    doc(db!, "users", user.uid),
    {
      growth: {
        ...
        advancedGrowthOps: Object.fromEntries(
          Object.entries(input.advancedGrowthOps ?? {}).map(([key, value]) => [
            key,
...
export async function getOwnedMerchProducts() {
  if (!auth?.currentUser || !db) return [];
  const snapshot = await getDocs(query(collection(db, "merchProducts"), where("ownerId", "==", auth.currentUser.uid), limit(50)));
...
export async function getCreatorMerchProducts(uid: string) {
  if (!db) return [];
  const snapshot = await getDocs(query(collection(db, "merchProducts"), where("ownerId", "==", uid), limit(50)));
  return snapshot.docs
    .map((docSnapshot: { id: string; data: () => Record<string, unknown> }) => {
      const data = docSnapshot.data();
      return {
...
export async function getIncomingPriorityInbox() {
  ...
  try {
    snapshot = await getDocs(query(collection(db, "priorityInbox"), where("creatorId", "==", auth.currentUser.uid), orderBy("createdAt", "desc"), limit(50)));
  } catch (error) {
    if (!isMissingIndexError(error)) {
      throw error;
    }

    snapshot = await getDocs(query(collection(db, "priorityInbox"), limit(100)));
  }
...
export async function getCreatorNewsletterIssues(creatorId: string) {
  ...
  try {
    snapshot = await getDocs(query(collection(db, "newsletterIssues"), where("creatorId", "==", creatorId), orderBy("createdAt", "desc"), limit(50)));
  } catch (error) {
    if (!isMissingIndexError(error)) {
      throw error;
    }

    snapshot = await getDocs(query(collection(db, "newsletterIssues"), limit(100)));
  }
...
export async function getCreatorBlogPosts(creatorId: string) {
  ...
  try {
    snapshot = await getDocs(query(collection(db, "blogPosts"), where("creatorId", "==", creatorId), orderBy("createdAt", "desc"), limit(50)));
  } catch (error) {
    if (!isMissingIndexError(error)) {
      throw error;
    }

    snapshot = await getDocs(query(collection(db, "blogPosts"), limit(100)));
  }
