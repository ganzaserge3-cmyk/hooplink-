...
export interface HealthOpsProfile {
  advancedHealthMeta: Record<string, string[]>;
}
...
export async function getHealthOpsProfile(): Promise<HealthOpsProfile> {
  if (!auth?.currentUser || !db) {
    return { advancedHealthMeta: {} };
  }

  const snapshot = await getDoc(doc(db, "healthOpsProfiles", auth.currentUser.uid));
  const data = snapshot.exists() ? (snapshot.data() as Record<string, unknown>) : {};
  const advancedHealthMeta = (data.advancedHealthMeta as Record<string, unknown> | undefined) ?? {};

  return {
    advancedHealthMeta: Object.fromEntries(
      Object.entries(advancedHealthMeta).map(([key, value]) => [
        key,
...
export async function saveHealthOpsProfile(input: HealthOpsProfile) {
  const user = requireUser();
  await setDoc(
    doc(db!, "healthOpsProfiles", user.uid),
    {
      ownerId: user.uid,
      advancedHealthMeta: Object.fromEntries(
        Object.entries(input.advancedHealthMeta).map(([key, value]) => [
          key,
