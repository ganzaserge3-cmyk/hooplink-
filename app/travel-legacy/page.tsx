...
import {
  ...
  saveTravelOpsProfile,
} from "@/lib/phase8-travel";

const ADVANCED_TRAVEL_FIELDS = [
  { key: "visaChecklist", label: "Visa checklist" },
  { key: "passportExpiryAlerts", label: "Passport expiry alerts" },
  { key: "packingChecklist", label: "Packing checklist" },
  { key: "roommateAssignmentBoard", label: "Roommate assignment board" },
  { key: "airportTransferBoard", label: "Airport transfer board" },
  { key: "travelApprovalWorkflow", label: "Travel approval workflow" },
  { key: "travelReimbursementTracker", label: "Travel reimbursement tracker" },
  { key: "mealStipendTracker", label: "Meal stipend tracker" },
  { key: "tripBudgetSummary", label: "Trip budget summary" },
  { key: "travelDocumentVault", label: "Travel document vault" },
  { key: "emergencyTripContacts", label: "Emergency trip contacts" },
  { key: "routePlanningAssistant", label: "Route planning assistant" },
  { key: "multiStopItineraryBuilder", label: "Multi-stop itinerary builder" },
  { key: "hotelCheckInStatus", label: "Hotel check-in status" },
  { key: "teamLuggageTracker", label: "Team luggage tracker" },
  { key: "travelIncidentReports", label: "Travel incident reports" },
  { key: "driverAssignmentBoard", label: "Driver assignment board" },
  { key: "teamTravelCostPerPlayerCalculator", label: "Team travel cost-per-player calculator" },
  { key: "parentPermissionSlipManager", label: "Parent permission slip manager" },
  { key: "travelReadinessScore", label: "Travel readiness score" },
  { key: "internationalTripPlanner", label: "International trip planner" },
  { key: "jetLagRecoveryGuide", label: "Jet lag recovery guide" },
] as const;
...
function TravelLegacyPageContent() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setAdvancedTravelOps(travelOpsProfile.advancedTravelOps);
  ...
  return (
    <ProtectedRoute>
      <div className="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <Card>
          <CardHeader><CardTitle>Travel and Logistics Ops</CardTitle><CardDescription>Track visa readiness, approvals, budget, transfers, incident notes, routing, and the wider trip-planning workflow.</CardDescription></CardHeader>
          <CardContent className="space-y-6">
            <div className="grid gap-3 md:grid-cols-4">
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{snapshot.visaAlerts.length}</div><div className="text-sm text-muted-foreground">Visa alerts</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{snapshot.reimbursements.length}</div><div className="text-sm text-muted-foreground">Travel reimbursements</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{snapshot.roommateAssignments.length}</div><div className="text-sm text-muted-foreground">Room assignments</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{Object.values(advancedTravelOps).reduce((sum, items) => sum + items.length, 0)}</div><div className="text-sm text-muted-foreground">Travel ops notes</div></div>
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              {ADVANCED_TRAVEL_FIELDS.map((field) => (
                <div key={field.key} className="space-y-2 rounded-xl border p-4">
                  <p className="font-medium">{field.label}</p>
                  <textarea
                    value={(advancedTravelOps[field.key] ?? []).join("\n")}
                    onChange={(event) => setAdvancedTravelOps((current) => ({
                      ...current,
            ...
            <Button variant="outline" onClick={() => void saveTravelOpsProfile({ advancedTravelOps }).then(() => setStatus("Travel and logistics workspace saved."))}>
              Save Travel Workspace
            </Button>
          </CardContent>
        </Card>
      </div>
