...
const ADVANCED_GROWTH_FIELDS = [
  { key: "fanReferralLeaderboard", label: "Fan referral leaderboard" },
  { key: "realTimeFundraisingThermometer", label: "Real-time fundraising thermometer" },
  { key: "merchInventoryLowStockAlerts", label: "Merch inventory low-stock alerts" },
] as const;
      ...
      <div className="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{inviteLeaderboard.length}</div><div className="text-sm text-muted-foreground">Referral leaders</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{fundraisers.length}</div><div className="text-sm text-muted-foreground">Fundraisers</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{Object.values(growth.advancedGrowthOps ?? {}).reduce((sum, items) => sum + items.length, 0)}</div><div className="text-sm text-muted-foreground">Growth ops notes</div></div>
            </div>

            <div className="grid gap-4 md:grid-cols-3">
              {ADVANCED_GROWTH_FIELDS.map((field) => (
                <div key={field.key} className="space-y-2 rounded-xl border p-4">
                  <p className="font-medium">{field.label}</p>
                  <textarea
                    value={(growth.advancedGrowthOps?.[field.key] ?? []).join("\n")}
                    onChange={(event) => setGrowth((current) => current ? {
                      ...current,
                      advancedGrowthOps: {
                        ...current.advancedGrowthOps,
