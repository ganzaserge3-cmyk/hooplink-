...
import {
  ...
  getCurrentUserPartnerReferralRecords,
...
function BillingPageContent() {
  ...
  const [partnerReferrals, setPartnerReferrals] = useState<PartnerReferralRecord[]>([]);
  const [taxReport, setTaxReport] = useState<TaxExportReport | null>(null);
  const [marketListingsCount, setMarketListingsCount] = useState(0);
  ...
  const [commissionForm, setCommissionForm] = useState({ partnerName: "", source: "", amountLabel: "" });
  const [partnerReferralForm, setPartnerReferralForm] = useState({ partnerName: "", referralCode: "", rewardLabel: "" });
  ...
  const refresh = async () => {
    ...
    setPartnerReferrals(nextPartnerReferrals);
    setTaxReport(nextTaxReport);
    setOrgBilling(nextOrgBilling);
    setMarketListingsCount(nextListings.length);
  };
      ...
      <div className="mx-auto max-w-7xl space-y-6 py-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage campaigns, invoices, subscriptions, bundles, commissions, referrals, payouts, and tax/export snapshots in one place.
          </p>
        </div>

        {payouts && ambassador && taxReport ? (
          <div className="grid gap-4 md:grid-cols-6">
            ...
            <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{marketListingsCount}</div><div className="text-sm text-muted-foreground">Store listings</div></div>
            ...
            <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">${taxReport.suggestedSetAside}</div><div className="text-sm text-muted-foreground">Suggested tax set-aside</div></div>
          </div>
        ) : null}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              <CardTitle>Campaigns, Contracts, and Invoices</CardTitle>
              <CardDescription>Run sponsorship deals and keep your creator billing pipeline organized.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              >
                ...
                <input value={campaignForm.budgetLabel} onChange={(event) => setCampaignForm((current) => ({ ...current, budgetLabel: event.target.value }))} placeholder="$500/video" className="h-11 rounded-md border border-input px-3 text-sm" />
                ...
                <textarea value={campaignForm.requirements} onChange={(event) => setCampaignForm((current) => ({ ...current, requirements: event.target.value }))} placeholder="Requirements, deliverables, timing" className="min-h-20 rounded-md border border-input px-3 py-2 text-sm" />
              ...
              <div className="space-y-3 border-t pt-4">
                {campaigns.slice(0, 3).map((campaign) => (
                  <div key={campaign.id} className="rounded-xl border p-3 text-sm">
                    <p className="font-semibold">{campaign.title}</p>
                    <p className="text-muted-foreground">{campaign.brandName} | {campaign.budgetLabel}</p>
                  </div>
                ))}
          ...
          <Card>
            <CardHeader>
              <CardTitle>Subscriptions, Discounts, and Bundles</CardTitle>
              <CardDescription>Set up promo codes, fan subscriptions, paid training plans, and team bundle pricing.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <form
                className="grid gap-3"
              ...
              >
                <input value={discountForm.code} onChange={(event) => setDiscountForm((current) => ({ ...current, code: event.target.value }))} placeholder="Promo code" className="h-10 rounded-md border border-input px-3 text-sm" />
              ...
              >
                <input value={tierForm.name} onChange={(event) => setTierForm((current) => ({ ...current, name: event.target.value }))} placeholder="Subscription tier" className="h-10 rounded-md border border-input px-3 text-sm" />
                ...
                <Button type="submit">Create Subscription Tier</Button>
              </form>
              ...
              >
                <input value={bundleForm.teamName} onChange={(event) => setBundleForm((current) => ({ ...current, teamName: event.target.value }))} placeholder="Bundle / team name" className="h-10 rounded-md border border-input px-3 text-sm" />
              ...
              <div className="space-y-2 border-t pt-4 text-sm">
                {discountCodes.map((code) => <div key={code.id} className="rounded-xl border p-3">{code.code} | {code.percentOff}% off | {code.uses}/{code.maxUses}</div>)}
                {tiers.map((tier) => <div key={tier.id} className="rounded-xl border p-3">{tier.name} | {tier.priceLabel} | {tier.checkoutProvider}</div>)}
                {bundles.map((bundle) => <div key={bundle.id} className="rounded-xl border p-3">{bundle.teamName} | {bundle.priceLabel} | {bundle.seats} seats</div>)}
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card>
            <CardHeader>
              <CardTitle>Commissions and Referrals</CardTitle>
              <CardDescription>Track affiliate payouts, partner commission lines, and referral program offers.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <form
                className="grid gap-3"
                onSubmit={async (event: FormEvent<HTMLFormElement>) => {
                  event.preventDefault();
                  await createCommissionRecord(commissionForm);
                  setCommissionForm({ partnerName: "", source: "", amountLabel: "" });
                  await refresh();
                }}
              >
                <input value={commissionForm.partnerName} onChange={(event) => setCommissionForm((current) => ({ ...current, partnerName: event.target.value }))} placeholder="Partner / brand" className="h-10 rounded-md border border-input px-3 text-sm" />
                <input value={commissionForm.source} onChange={(event) => setCommissionForm((current) => ({ ...current, source: event.target.value }))} placeholder="Affiliate source" className="h-10 rounded-md border border-input px-3 text-sm" />
                <input value={commissionForm.amountLabel} onChange={(event) => setCommissionForm((current) => ({ ...current, amountLabel: event.target.value }))} placeholder="$35" className="h-10 rounded-md border border-input px-3 text-sm" />
                <Button type="submit">Track Commission</Button>
              </form>
              ...
              <form
                ...
                onSubmit={async (event: FormEvent<HTMLFormElement>) => {
                  event.preventDefault();
                  await createPartnerReferralRecord(partnerReferralForm);
                  setPartnerReferralForm({ partnerName: "", referralCode: "", rewardLabel: "" });
                  await refresh();
                }}
              >
                <input value={partnerReferralForm.partnerName} onChange={(event) => setPartnerReferralForm((current) => ({ ...current, partnerName: event.target.value }))} placeholder="Partner name" className="h-10 rounded-md border border-input px-3 text-sm" />
                <input value={partnerReferralForm.referralCode} onChange={(event) => setPartnerReferralForm((current) => ({ ...current, referralCode: event.target.value.toUpperCase() }))} placeholder="Referral code" className="h-10 rounded-md border border-input px-3 text-sm" />
                <input value={partnerReferralForm.rewardLabel} onChange={(event) => setPartnerReferralForm((current) => ({ ...current, rewardLabel: event.target.value }))} placeholder="Reward label" className="h-10 rounded-md border border-input px-3 text-sm" />
                <Button type="submit">Create Referral Offer</Button>
              </form>

              <div className="space-y-2 border-t pt-4 text-sm">
                {commissions.map((entry) => <div key={entry.id} className="rounded-xl border p-3">{entry.partnerName} | {entry.source} | {entry.amountLabel} | {entry.status}</div>)}
                {partnerReferrals.map((entry) => <div key={entry.id} className="rounded-xl border p-3">{entry.partnerName} | {entry.referralCode} | {entry.rewardLabel}</div>)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Sponsor verification requests and content-approval queue for campaign work.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <form
                className="grid gap-3"
              ...
              <div className="space-y-2 border-t pt-4 text-sm">
                {badgeRequests.map((request) => <div key={request.id} className="rounded-xl border p-3">{request.brandName} | {request.website} | {request.status}</div>)}
          ...
          <Card>
            <CardHeader>
              <CardTitle>Org Controls and Export</CardTitle>
              <CardDescription>Billing controls, fan subscription rails, payout toggles, and export-ready finance snapshot.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {orgBilling ? (
                <form
              ...
              {taxReport ? (
                <div className="rounded-2xl border p-4 text-sm">
                  <p className="font-semibold">Tax and export snapshot</p>
                  <p className="mt-2 text-muted-foreground">Taxable revenue: ${taxReport.estimatedTaxableRevenue}</p>
                  <p className="text-muted-foreground">Invoices: ${taxReport.invoiceRevenue} | Bookings: ${taxReport.bookingRevenue}</p>
                  <p className="text-muted-foreground">Tips: ${taxReport.tipRevenue} | Listings: ${taxReport.listingRevenue}</p>
                  <p className="mt-2 text-muted-foreground">Export label: {taxReport.exportLabel}</p>
                </div>
              ) : null}

              <div className="rounded-2xl border p-4 text-sm">
                <p className="font-semibold">Partner referral program</p>
                <p className="mt-2 text-muted-foreground">Top ambassador code: {ambassador?.topInviteCode || "No code yet"}</p>
                <p className="text-muted-foreground">Invite codes created: {ambassador?.invitesCreated ?? 0}</p>
                <p className="text-muted-foreground">Referral conversions tracked: {ambassador?.referralCount ?? 0}</p>
              </div>
            </CardContent>
          </Card>
        </div>
