...
const ADVANCED_ADMIN_FIELDS = [
  { key: "safetyReportingCenter", label: "Safety reporting center" },
  { key: "moderationQueue", label: "Moderation queue" },
  { key: "contentTakedownWorkflow", label: "Content takedown workflow" },
  { key: "verifiedOrgPages", label: "Verified org pages" },
  { key: "adminDashboardWidgets", label: "Admin dashboard" },
  { key: "orgBillingConsole", label: "Org billing console" },
  { key: "securityAlertsBoard", label: "Security alerts" },
  { key: "loginRiskAlerts", label: "Login risk alerts" },
  { key: "deviceSessionManager", label: "Device session manager" },
  { key: "auditLogOps", label: "Audit logs" },
] as const;
...
function AdminPageContent() {
  ...
  const [advancedAdminOps, setAdvancedAdminOps] = useState<Record<string, string[]>>({});

  useEffect(() => {
    void getAppAccessSettings().then(setSettings);
    void getAdminWorkspaceProfile().then((profile) => setAdvancedAdminOps(profile.advancedAdminOps));
  ...
  return (
    <ProtectedRoute>
      <div className="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Advanced Safety and Admin Controls</CardTitle>
            <CardDescription>Track deeper admin workflows for reports, takedowns, org verification, billing, session review, and security operations.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid gap-3 md:grid-cols-4">
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{alerts.length}</div><div className="text-sm text-muted-foreground">Security alerts</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{auditLogs.length}</div><div className="text-sm text-muted-foreground">Audit events</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{users.filter((entry) => entry.verified).length}</div><div className="text-sm text-muted-foreground">Verified users</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{Object.values(advancedAdminOps).reduce((sum, items) => sum + items.length, 0)}</div><div className="text-sm text-muted-foreground">Admin ops notes</div></div>
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              {ADVANCED_ADMIN_FIELDS.map((field) => (
                <div key={field.key} className="space-y-2 rounded-xl border p-4">
                  <p className="font-medium">{field.label}</p>
                  <textarea
                    value={(advancedAdminOps[field.key] ?? []).join("\n")}
                    onChange={(event) => setAdvancedAdminOps((current) => ({
                      ...current,
            ...
            <Button variant="outline" onClick={() => void saveAdminWorkspaceProfile({ advancedAdminOps })}>
              Save Advanced Admin Controls
            </Button>
          </CardContent>
        </Card>
      </div>
