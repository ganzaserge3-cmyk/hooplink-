...
import {
  ...
  loadOfflineDraft,
  saveOfflineDraft,
  saveFuturePremiumProfile,
  saveQuietHoursSettings,
  saveWearableSync,
  type FieldMetricRecord,
  type FuturePremiumProfile,
...
const ADVANCED_FUTURE_FIELDS = [
  { key: "arPlayerCards", label: "AR player cards" },
  { key: "arVenueNavigation", label: "AR venue navigation" },
  { key: "vrFilmRoom", label: "VR film room" },
  { key: "wearableDeviceSync", label: "Wearable device sync" },
  { key: "smartwatchAlerts", label: "Smartwatch alerts" },
  { key: "voiceControlledCoachingBoard", label: "Voice-controlled coaching board" },
  { key: "aiRecruitingSimulation", label: "AI recruiting simulation" },
  { key: "aiOpponentScoutingBot", label: "AI opponent scouting bot" },
  { key: "aiContentCalendarAutopilot", label: "AI content calendar autopilot" },
  { key: "aiSponsorFitMatcher", label: "AI sponsor-fit matcher" },
  { key: "threeDShotTrajectoryViewer", label: "3D shot trajectory viewer" },
  { key: "smartCameraAutoFollow", label: "Smart camera auto-follow" },
  { key: "biometricPerformanceSync", label: "Biometric performance sync" },
  { key: "digitalCollectibleLocker", label: "NFT-free digital collectible locker" },
  { key: "fanReputationEngine", label: "Fan reputation engine" },
  { key: "creatorTalentMarketplace", label: "Creator talent marketplace" },
  { key: "automatedScoutDigest", label: "Automated scout digest" },
  { key: "franchiseOperationsSuite", label: "Franchise operations suite" },
  { key: "leagueCommissionerCenter", label: "League commissioner center" },
  { key: "fullSportsOperatingSystem", label: "Full sports operating system" },
] as const;
...
function IntelligencePageContent() {
  ...
  const [futureProfile, setFutureProfile] = useState<FuturePremiumProfile>({ advancedFutureOps: {} });
  ...
  const refresh = async () => {
    const [nextWearables, nextMetrics, nextMorale, nextQuietHours, nextProfiles, nextSeason, nextFutureProfile] = await Promise.all([
      getWearableSyncs(),
      getFieldMetrics(),
      getMoralePulses(),
      getQuietHoursSettings(),
    ...
    setFutureProfile(nextFutureProfile);
  };
  ...
  return (
    <ProtectedRoute>
      <div className="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Future and Premium Innovation Lab</CardTitle>
            <CardDescription>Track AR, VR, smart devices, AI automation, scouting intelligence, and league-scale premium ideas in one saved workspace.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid gap-3 md:grid-cols-4">
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{wearables.length}</div><div className="text-sm text-muted-foreground">Connected devices</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{metrics.length}</div><div className="text-sm text-muted-foreground">Field metrics</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{profiles.filter((profile) => profile.role?.type === "scout").length}</div><div className="text-sm text-muted-foreground">Scout profiles</div></div>
              <div className="rounded-xl border p-4 text-center"><div className="text-2xl font-bold">{Object.values(futureProfile.advancedFutureOps).reduce((sum, items) => sum + items.length, 0)}</div><div className="text-sm text-muted-foreground">Premium ops notes</div></div>
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              {ADVANCED_FUTURE_FIELDS.map((field) => (
                <div key={field.key} className="space-y-2 rounded-xl border p-4">
                  <p className="font-medium">{field.label}</p>
                  <textarea
                    value={(futureProfile.advancedFutureOps[field.key] ?? []).join("\n")}
                    onChange={(event) => setFutureProfile((current) => ({
                      ...current,
                      advancedFutureOps: {
                        ...current.advancedFutureOps,
            ...
            <Button variant="outline" onClick={() => void saveFuturePremiumProfile(futureProfile).then(() => setStatus("Future and premium workspace saved."))}>
