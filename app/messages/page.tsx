            ...
            {activeConversation ? (
              <>
                ...
                <div className="flex-1 space-y-3 overflow-y-auto p-4">
                  {messages.length === 0 ? (
                    <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Say hello ≡ƒæï</div>
                  ) : messages.map((msg) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex ${own ? "justify-end" : "justify-start"}`}>
                        <div className="max-w-[75%] space-y-1">
                          {msg.replyTo && (
                            <div className="rounded-xl border bg-muted/40 px-3 py-1.5 text-xs text-muted-foreground">Γå⌐ {msg.replyTo.text}</div>
                          )}
                ...
                <form onSubmit={handleSend} className="border-t p-3">
                  {replyTo && (
                    <div className="mb-2 flex items-center justify-between rounded-xl bg-muted px-3 py-2 text-xs">
                      <span className="truncate">Γå⌐ {replyTo.text}</span>
                      <button type="button" onClick={() => setReplyTo(null)} className="ml-2 text-muted-foreground">Γ£ò</button>
                    </div>
                  )}
                  {attachment && (
                    <div className="mb-2 flex items-center justify-between rounded-xl bg-muted px-3 py-2 text-xs">
                      <span className="truncate">{attachment.name}</span>
                      <button type="button" onClick={() => setAttachment(null)} className="ml-2 text-muted-foreground">Γ£ò</button>
                    </div>
                  )}
